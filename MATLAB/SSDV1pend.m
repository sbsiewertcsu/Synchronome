...
% Once (YOLO v2), and SSD. This example trains an SSD pend detector using the 
...
%end
%% Load Dataset
% This example uses a small pend data set that contains 295 images. Each 
% image contains one or two labeled instances of a zero. A small data set is 
% useful for exploring the SSD training procedure, but in practice, more labeled 
% images are needed to train a robust detector.

%unzip pendDatasetImages.zip
data = load('pendDatasetGroundTruth.mat');
pendDataset = data.gTruth;
%% 
% The training data is stored in a table. The first column contains the path 
% to the image files. The remaining columns contain the ROI labels for pends. 
...
rng(0);
shuffledIndices = randperm(height(pendDataset));
idx = floor(0.6 * length(shuffledIndices) );
trainingData = pendDataset(shuffledIndices(1:idx),:);
testData = pendDataset(shuffledIndices(idx+1:end),:);
%% 
...
bldsTrain = boxLabelDatastore(trainingData(:,'zero'));

imdsTest = imageDatastore(testData{:,'imageFilename'});
bldsTest = boxLabelDatastore(testData(:,'zero'));
%% 
...
numClasses = width(pendDataset)-1;
%% 
